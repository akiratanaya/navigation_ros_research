...
class CmuSimBridge(Node):
    def __init__(self):
        ...
        # Periodic timer for boundary publishing (1 Hz)
    ...
    def _publish_static_transforms(self):
        ...
        # Note: /sensor -> /vehicle and /sensor -> /camera are already published by local_planner.launch.
        # Do not publish vehicle -> sensor here to avoid cyclic TF tree conflict.
    ...
    def publish_boundaries(self):
        ...
        # Arena interior boundary (11m x 11m arena with 0.15m perimeter walls at +/-5.47m)
        coords = [
            (-5.2, -5.2),
            (5.2, -5.2),
            (5.2, 5.2),
    ...
    def points_callback(self, msg: PointCloud2):
        ...
        try:
            ...
            self.get_logger().warn(f"TF lookup failed for {msg.header.frame_id}: {e}", throttle_duration_sec=2.0)
            return
        ...
        # Filter out rays that fly over the 0.15m perimeter wall into open space outside the arena
        mask = (np.abs(pts_world[:, 0]) <= 5.40) & (np.abs(pts_world[:, 1]) <= 5.40)
